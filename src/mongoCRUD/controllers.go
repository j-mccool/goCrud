...
func createProfile(w http.ResponseWriter, r *http.Request) {
	...
	var person user
	err := json.NewDecoder(r.Body).Decode(&person) // storing in person variable type user
	if err != nil {
		fmt.Print(err)
	}
...
func updateProfile(w http.ResponseWriter, r *http.Request) {
	...
	filter := bson.D{{"name", body.Name}} //converting vlaue to BSON type
	after := options.After
...
func deleteProfile(w http.ResponseWriter, r *http.Request) {
	...
	opts := options.Delete().SetCollation(&options.Collation{}) // to specify language specific rules for string comparison, like letter case
...
func getAllUsers(w http.ResponseWriter, r *http.Request) {
	...
	cur.Close(context.TODO()) //close the curso once stream of documents has exhausted
